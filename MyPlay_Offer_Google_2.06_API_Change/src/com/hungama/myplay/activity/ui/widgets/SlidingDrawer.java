...
public class SlidingDrawer extends ViewGroup {
    ...
    private final Rect mHandleHitRect = new Rect();
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        int handleClickX = (int)(event.getX() - mHandleLayout.getTop());
        int handleClickY = (int)(event.getY() - mHandleLayout.getLeft());

        // checks if the handle was clicked.
    	if (mExpanded && action == MotionEvent.ACTION_DOWN && 
    			interceptClickFromParentRecursive(mHandleLayout, handleClickX, handleClickY)) {
    		return false;
    	}
...
//        Logger.v(VIEW_LOG_TAG, "Touch Event: X: " + x + " Y: " + y);
//        Logger.v(VIEW_LOG_TAG, "Is Tracking: " + mTracking);
//        Logger.v(VIEW_LOG_TAG, "Handle:  L: " + handle.getLeft() + " T: " + handle.getTop() + 
//        						" R: " + handle.getRight() + " B: " + handle.getBottom());
    ...
    private boolean interceptClickFromParentRecursive(ViewGroup parent, int parentClickX, int parentClickY) {
    	...
    	for (int i=0;i<childCount;i++) {
    		...
    		if (childView.getVisibility() != View.GONE || childView.getVisibility() != View.INVISIBLE) {
    			if (childView instanceof ViewGroup) {
    				childViewGroup = (ViewGroup) childView;
    				boolean handled = interceptClickFromParentRecursive(childViewGroup, parentClickX, parentClickY);
    				if (handled) {
    					return true;
    				}
    			} else {
    				if (childView.isClickable()) {
    					...
    					if (hitRect.contains(parentClickX, parentClickY)) {
    						// Call the view's onTouchEvent with new EventMotion.
...
//		} else {
//			// no hardware accelerated before.
//			content.buildDrawingCache();
//		}
    ...
    public void moveHandleTo(int position) {
    	moveHandle(position);
    }
